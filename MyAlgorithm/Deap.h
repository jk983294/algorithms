...
public:
    ...
    void print(){
        for(int i=2;i<=1+size;i++){cout<<heap[i]<<" ";}
        cout<<endl;
    };
public:
...
void Deap<T>::insertKey(T x){
    size ++;
    if(size == 1) { heap[2] = x;  return;}    //first element
...
bool Deap<T>::isMaxHeap(int p){
    ...
    if((p<pow(2,level)) && (p >= pow(2,level) - pow(2,level-2)))    return true;
    else return false;
}
template <class T>
...
int Deap<T>::MaxPartner(int p){
    int level = ceil(log(p+1)/log(2));
    if((p+pow(2,level-2)) <= size + 1) return p+pow(2,level-2); //对应右子树节点存在
    else return (p+pow(2,level-2))/2;   //不存在，返回对应节点的双亲位置
}
template <class T>
...
bool Deap<T>::deleteMax(T &ret){
    bool isSuccess = false;
    if(size == 1){
        ret = heap[2];  size--; isSuccess = true;
    }
    else if(size > 1){
        ret = heap[3];
        T last = heap[size + 1];
