...
bool MaxLeftistTree<T>::deleteKey(T &ret){
    ...
    if(BinaryTree<T>::root != NULL){
        ...
        delete a;
        BinaryTree<T>::size--; isSuccess = true;
    }
    return isSuccess;
}
template <class T>
void MaxLeftistTree<T>::combine(MaxLeftistTree<T> &b){
    if(BinaryTree<T>::size == 0)  {    BinaryTree<T>::root = b.root;  }
    else if(b.size > 0) {  BinaryTree<T>::root = Union(BinaryTree<T>::root, b.root);}
...
TreeNode<T>* MaxLeftistTree<T>::Union(TreeNode<T>*a, TreeNode<T> *b){
    ...
    else a->right = Union(a->right, b);
    //维护左偏树性质
    if(a->left == NULL){    //左树为空，交换
        a->left = a->right;
        a->right = NULL;
    }
    else if(static_cast<LeftistNode<T>*>(a->left)->shortest < static_cast<LeftistNode<T>*>(a->right)->shortest){    //swap
        tmp = a->left;    a->left=a->right;    a->right= tmp;
    }
    //set shortest
    if(a->right == NULL)    (static_cast<LeftistNode<T>*>(a))->shortest = 1;
    else (static_cast<LeftistNode<T>*>(a))->shortest = (static_cast<LeftistNode<T>*>(a->right))->shortest + 1;
    return a;
}
template <class T>
void MaxLeftistTree<T>::insert(T data){//将data插入树中orgin的左边或右边
    LeftistNode<T> *b = new  LeftistNode<T>(data);
    MaxLeftistTree<T> tree;
    tree.root = b;  tree.size = 1;
    combine(tree);
}
...
template <class T>
void MinLeftistTree<T>::insert(T data){//将data插入树中orgin的左边或右边
    LeftistNode<T> *b = new  LeftistNode<T>(data);
