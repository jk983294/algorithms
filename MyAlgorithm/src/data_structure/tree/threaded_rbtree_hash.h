...
struct is_trivial_expand<std::pair<K, V>>
    : public std::conditional<std::is_trivial<K>::value && std::is_trivial<V>::value, std::true_type,
                              std::false_type>::type {};
...
void move_construct_and_destroy(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin,
                                move_trivial_tag) {
...
protected:
    ...
    struct pod_value_t {
        ...
        storage_type *value() { return reinterpret_cast<storage_type *>(&value_pod); }
        storage_type const *value() const { return reinterpret_cast<storage_type const *>(&value_pod); }
    };
    ...
    struct root_t : public hasher,
        ...
        root_t(any_hasher &&hash, any_key_compare &&compare, any_allocator_type &&alloc)
            ...
            static_assert(std::is_unsigned<offset_type>::value && std::is_integral<offset_type>::value,
    ...
    struct deref_node_t {
        node_t &operator()(size_type index) const { return root_ptr->node[index]; }
        root_t const *root_ptr;
    };
    struct const_deref_node_t {
        node_t const &operator()(size_type index) const { return root_ptr->node[index]; }
        root_t const *root_ptr;
    };
    struct deref_key_t {
        key_type const &operator()(size_type index) const { return config_t::get_key(*root_ptr->value[index].value()); }
        root_t const *root_ptr;
    };
...
public:
    ...
    public:
        ...
        reference operator*() const { return reinterpret_cast<reference>(*self->root_.value[offset].value()); }
        pointer operator->() const { return reinterpret_cast<pointer>(self->root_.value[offset].value()); }
        bool operator==(iterator const &other) const { return offset == other.offset && self == other.self; }
        bool operator!=(iterator const &other) const { return offset != other.offset || self != other.self; }
    ...
    public:
        ...
        const_pointer operator->() const { return reinterpret_cast<const_pointer>(self->root_.value[offset].value()); }
        bool operator==(const_iterator const &other) const { return offset == other.offset && self == other.self; }
        bool operator!=(const_iterator const &other) const { return offset != other.offset || self != other.self; }
    ...
    public:
        ...
        reference operator*() const { return reinterpret_cast<reference>(*self->root_.value[offset].value()); }
        pointer operator->() const { return reinterpret_cast<pointer>(self->root_.value[offset].value()); }
        bool operator==(local_iterator const &other) const { return offset == other.offset && self == other.self; }
        bool operator!=(local_iterator const &other) const { return offset != other.offset || self != other.self; }
...
protected:
    ...
    typename std::enable_if<unique_type::value, insert_result_t>::type result_(pair_posb_t posb) {
...
public:
    ...
    // empty
    explicit threaded_rbtree_hash(size_type bucket_count, hasher const &hash = hasher(),
    ...
    // empty
    threaded_rbtree_hash(size_type bucket_count, hasher const &hash, allocator_type const &alloc)
        : root_(hash, key_compare(), alloc) {
        rehash(bucket_count);
    }
    // range
    template <class iterator_t>
    threaded_rbtree_hash(iterator_t begin, iterator_t end, size_type bucket_count = 8, hasher const &hash = hasher(),
                         key_compare const &compare = key_compare(), allocator_type const &alloc = allocator_type())
        : root_(hash, compare, alloc) {
        rehash(bucket_count);
        insert(begin, end);
    }
    // range
    template <class iterator_t>
    threaded_rbtree_hash(iterator_t begin, iterator_t end, size_type bucket_count, allocator_type const &alloc)
    ...
    // range
    template <class iterator_t>
    threaded_rbtree_hash(iterator_t begin, iterator_t end, size_type bucket_count, hasher const &hash,
    ...
    // copy
    threaded_rbtree_hash(threaded_rbtree_hash const &other, allocator_type const &alloc)
    ...
    // move
    threaded_rbtree_hash(threaded_rbtree_hash &&other, allocator_type const &alloc)
        : root_(std::move(other.get_hasher()), std::move(other.get_key_comp()), alloc) {
        copy_all_<true>(&other.root_);
    }
    // initializer list
    threaded_rbtree_hash(std::initializer_list<value_type> il, size_type bucket_count = 8,
                         hasher const &hash = hasher(), key_compare const &compare = key_compare(),
                         allocator_type const &alloc = allocator_type())
        : threaded_rbtree_hash(il.begin(), il.end(), std::distance(il.begin(), il.end()), hash, compare, alloc) {}
    // initializer list
    threaded_rbtree_hash(std::initializer_list<value_type> il, size_type bucket_count, allocator_type const &alloc)
        : threaded_rbtree_hash(il.begin(), il.end(), std::distance(il.begin(), il.end()), alloc) {}
    // initializer list
    threaded_rbtree_hash(std::initializer_list<value_type> il, size_type bucket_count, hasher const &hash,
                         allocator_type const &alloc)
        : threaded_rbtree_hash(il.begin(), il.end(), std::distance(il.begin(), il.end()), hash, alloc) {}
    // destructor
    ~threaded_rbtree_hash() { dealloc_all_(); }
    // copy
    ...
    allocator_type get_allocator() const { return *static_cast<value_allocator_t const *>(&root_); }
    hasher hash_function() const { return *static_cast<hasher const *>(&root_); }
    key_compare key_comp() const { return *static_cast<key_compare const *>(&root_); }

    void swap(threaded_rbtree_hash &other) { std::swap(root_, other.root_); }
    ...
    // with hint
    iterator insert(const_iterator hint, value_type const &value) { return iterator(insert_value_(value).first, this); }
    // with hint
    template <class in_value_t>
    typename std::enable_if<std::is_convertible<in_value_t, value_type>::value, insert_result_t>::type insert(
    ...
    pair_lili_t equal_range(key_type const &key) {
        ...
        threaded_rbtree_equal_range(root_.bucket[bucket], const_deref_node_t{&root_}, key, deref_key_t{&root_},
                                    get_key_comp(), lower, upper);
    ...
    local_iterator begin(size_type n) { return local_iterator(root_.bucket[n].root.root, this); }
    local_iterator end(size_type n) { return local_iterator(offset_empty, this); }
    const_local_iterator begin(size_type n) const { return const_local_iterator(root_.bucket[n].root.root, this); }
    const_local_iterator end(size_type n) const { return const_local_iterator(offset_empty, this); }
    const_local_iterator cbegin(size_type n) const { return const_local_iterator(root_.bucket[n].root.root, this); }
    const_local_iterator cend(size_type n) const { return const_local_iterator(offset_empty, this); }

    size_type bucket_count() const { return root_.bucket_count; }
    size_type max_bucket_count() const { return max_size(); }

    size_type bucket_size(size_type n) const { return std::distance(begin(n), end(n)); }
...
protected:
    ...
    static void construct_one_(iterator_t where, args_t &&... args) {
        threaded_rbtree_hash_detail::construct_one(
            where, typename threaded_rbtree_hash_detail::get_tag<iterator_t>::type(), std::forward<args_t>(args)...);
    }

    template <class iterator_t>
    static void destroy_one_(iterator_t where) {
        threaded_rbtree_hash_detail::destroy_one(where,
                                                 typename threaded_rbtree_hash_detail::get_tag<iterator_t>::type());
    }

    template <class iterator_from_t, class iterator_to_t>
    static void move_construct_and_destroy_(iterator_from_t move_begin, iterator_from_t move_end,
                                            iterator_to_t to_begin) {
        threaded_rbtree_hash_detail::move_construct_and_destroy(
            move_begin, move_end, to_begin, typename threaded_rbtree_hash_detail::get_tag<iterator_from_t>::type());
    }
        ...
        if (size > 0) {
            ...
            for (size_type other_i = 0; other_i < other->size; ++other_i) {
                if (other->node[other_i].is_used()) {
                    ...
                    stack_t stack;
                    threaded_rbtree_find_path_for_multi(root_.bucket[bucket], stack, deref_node_t{&root_}, i,
                                                        get_offset_comp());
        ...
        if (root_.bucket_count != 0) {
            for (size_type i = 0; i < root_.size; ++i) {
                if (root_.node[i].is_used()) {
                    ...
                    stack_t stack;
                    threaded_rbtree_find_path_for_multi(new_bucket[bucket], stack, const_deref_node_t{&root_}, i,
                                                        get_offset_comp());
    ...
    void realloc_(size_type size) {
        ...
        if (size * value_size > 0x1000) {
            size = ((size * value_size + std::max<size_type>(value_size, 0x1000) - 1) & (~size_type(0) ^ 0xFFF)) /
                   value_size;
        } else if (size * value_size > 0x100) {
            size = ((size * value_size + std::max<size_type>(value_size, 0x100) - 1) & (~size_type(0) ^ 0xFF)) /
                   value_size;
        } else {
            size =
                ((size * value_size + std::max<size_type>(value_size, 0x10) - 1) & (~size_type(0) ^ 0xF)) / value_size;
        }
        ...
        if (root_.capacity != 0) {
            ...
            move_construct_and_destroy_(root_.value->value(), root_.value->value() + root_.capacity,
                                        new_value->value());
            get_node_allocator_().deallocate(root_.node, root_.capacity);
            get_value_allocator_().deallocate(root_.value, root_.capacity);
        }
    ...
    typename std::enable_if<std::is_same<key_type, storage_type>::value &&
                                !std::is_same<typename std::remove_reference<in_t>::type, key_type>::value,
                            pair_posb_t>::type
    insert_value_uncheck_(std::true_type, in_t &&in, args_t &&... args) {
        key_type key = get_key_t()(in, args...);
        size_type bucket = get_hasher()(key) % root_.bucket_count;
        stack_t stack;
        if (threaded_rbtree_find_path_for_unique(root_.bucket[bucket], stack, deref_node_t{&root_}, key,
                                                 deref_key_t{&root_}, get_key_comp())) {
    ...
    template <class in_t, class... args_t>
    pair_posb_t insert_value_uncheck_(std::false_type, in_t &&in, args_t &&... args) {
        size_type offset = root_.free_list == offset_empty ? root_.size : root_.free_list;
        construct_one_(root_.value[offset].value(), std::forward<in_t>(in), std::forward<args_t>(args)...);
        if (offset == root_.free_list) {
            root_.free_list = root_.node[offset].left_get_link();
        ...
        stack_t stack;
        threaded_rbtree_find_path_for_multi(root_.bucket[bucket], stack, deref_node_t{&root_}, offset,
                                            get_offset_comp());
        threaded_rbtree_insert(root_.bucket[bucket], stack, deref_node_t{&root_}, offset);
        return {offset, true};
    }
    ...
    size_type find_value_(key_type const &key) const {
        size_type bucket = get_hasher()(key) % root_.bucket_count;
        size_type offset = threaded_rbtree_lower_bound(root_.bucket[bucket], deref_node_t{&root_}, key,
                                                       deref_key_t{&root_}, get_key_comp());
        return (offset == offset_empty || get_key_comp()(key, get_key_t()(*root_.value[offset].value()))) ? root_.size
                                                                                                          : offset;
    }
    ...
    void remove_offset_(size_type offset) {
        size_type bucket = get_hasher()(get_key_t()(*root_.value[offset].value())) % root_.bucket_count;
        stack_t stack;
        threaded_rbtree_find_path_for_remove(root_.bucket[bucket], stack, deref_node_t{&root_}, offset,
                                             get_offset_comp());
        threaded_rbtree_remove(root_.bucket[bucket], stack, deref_node_t{&root_});
        destroy_one_(root_.value[offset].value());
        root_.node[offset].set_empty();
...
template <class key_t, class hasher_t = std::hash<key_t>, class key_compare_t = std::less<key_t>,
          class allocator_t = std::allocator<key_t>>
using trb_hash_set = threaded_rbtree_hash<
    threaded_rbtree_hash_set_config_t<key_t, std::true_type, hasher_t, key_compare_t, allocator_t>>;

template <class key_t, class hasher_t = std::hash<key_t>, class key_compare_t = std::less<key_t>,
          class allocator_t = std::allocator<key_t>>
using trb_hash_multiset = threaded_rbtree_hash<
    threaded_rbtree_hash_set_config_t<key_t, std::false_type, hasher_t, key_compare_t, allocator_t>>;

template <class key_t, class value_t, class hasher_t = std::hash<key_t>, class key_compare_t = std::less<key_t>,
          class allocator_t = std::allocator<std::pair<key_t const, value_t>>>
class trb_hash_map
    : public threaded_rbtree_hash<
          threaded_rbtree_hash_map_config_t<key_t, value_t, std::true_type, hasher_t, key_compare_t, allocator_t>> {
    typedef threaded_rbtree_hash<
        threaded_rbtree_hash_map_config_t<key_t, value_t, std::true_type, hasher_t, key_compare_t, allocator_t>>
        base_t;
...
template <class key_t, class value_t, class hasher_t = std::hash<key_t>, class key_compare_t = std::less<key_t>,
          class allocator_t = std::allocator<std::pair<key_t const, value_t>>>
using trb_hash_multimap = threaded_rbtree_hash<
    threaded_rbtree_hash_map_config_t<key_t, value_t, std::false_type, hasher_t, key_compare_t, allocator_t>>;
