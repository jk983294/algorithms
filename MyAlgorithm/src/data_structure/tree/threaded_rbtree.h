...
struct threaded_rbtree_node_t {
    ...
    void left_set_link(std::size_t link) { children[0] = index_type((children[0] & full_bit_mask) | link); }

    void right_set_link(std::size_t link) { children[1] = index_type((children[1] & full_bit_mask) | link); }
...
struct threaded_rbtree_root_t {
    ...
    struct root_type : public threaded_rbtree_count_t<node_t, enable_count_t>,
...
bool threaded_rbtree_find_path_for_unique(root_t &root,
    ...
    return threaded_rbtree_find_path_for_unique(root, stack, deref, key, deref_key, comparator, comparator_3way());
}
...
bool threaded_rbtree_find_path_for_remove(root_t &root,
    ...
    return threaded_rbtree_find_path_for_remove(root, stack, deref, index, comparator, comparator_3way());
}
...
std::size_t threaded_rbtree_lower_bound(root_t &root, deref_node_t deref, key_t const &key, deref_key_t deref_key,
                                        comparator_t comparator) {
    typedef typename root_t::node_type node_type;
...
template <class root_t, class comparator_t, class key_t, class deref_node_t, class deref_key_t>
std::size_t threaded_rbtree_equal_unique(root_t &root, deref_node_t deref, key_t const &key, deref_key_t deref_key,
                                         comparator_t comparator) {
    typedef typename threded_rb_tree_tools::has_compare<comparator_t, std::size_t>::type comparator_3way;
    return threaded_rbtree_equal_unique(root, deref, key, deref_key, comparator, comparator_3way());
}
template <class root_t, class comparator_t, class key_t, class deref_node_t, class deref_key_t>
std::size_t threaded_rbtree_equal_unique(root_t &root, deref_node_t deref, key_t const &key, deref_key_t deref_key,
                                         comparator_t comparator, std::false_type) {
...
template <class root_t, class comparator_t, class key_t, class deref_node_t, class deref_key_t>
std::size_t threaded_rbtree_reverse_lower_bound(root_t &root, deref_node_t deref, key_t const &key,
                                                deref_key_t deref_key, comparator_t comparator) {
    typedef typename root_t::node_type node_type;
...
template <class root_t, class comparator_t, class key_t, class deref_node_t, class deref_key_t>
std::size_t threaded_rbtree_upper_bound(root_t &root, deref_node_t deref, key_t const &key, deref_key_t deref_key,
                                        comparator_t comparator) {
    typedef typename root_t::node_type node_type;
...
template <class root_t, class comparator_t, class key_t, class deref_node_t, class deref_key_t>
std::size_t threaded_rbtree_reverse_upper_bound(root_t &root, deref_node_t deref, key_t const &key,
                                                deref_key_t deref_key, comparator_t comparator) {
    typedef typename root_t::node_type node_type;
...
template <class root_t, class comparator_t, class key_t, class deref_node_t, class deref_key_t>
void threaded_rbtree_equal_range(root_t &root, deref_node_t deref, key_t const &key, deref_key_t deref_key,
    ...
    p = upper == node_type::nil_sentinel
            ? root.root.root
            : deref(upper).left_is_child() ? deref(upper).left_get_link() : node_type::nil_sentinel;
    while (p != node_type::nil_sentinel) {
        if (comparator(key, deref_key(p))) {
            upper = p;
            if (deref(p).left_is_thread()) {
...
template <class root_t, class comparator_t, class key_t, class deref_node_t, class deref_key_t>
void threaded_rbtree_reverse_equal_range(root_t &root, deref_node_t deref, key_t const &key, deref_key_t deref_key,
                                         comparator_t comparator, std::size_t &lower, std::size_t &upper) {
    typedef typename root_t::node_type node_type;
    ...
    p = upper == node_type::nil_sentinel
            ? root.root.root
            : deref(upper).right_is_child() ? deref(upper).right_get_link() : node_type::nil_sentinel;
    while (p != node_type::nil_sentinel) {
        if (comparator(deref_key(p), key)) {
            upper = p;
            if (deref(p).right_is_thread()) {
...
void threaded_rbtree_insert(root_t &root, threaded_rbtree_stack_t<typename root_t::node_type, max_depth> &stack,
...
template <class root_t, class dereference_t, size_t max_depth>
void threaded_rbtree_remove(root_t &root, threaded_rbtree_stack_t<typename root_t::node_type, max_depth> &stack,
                            dereference_t deref) {
    typedef typename root_t::node_type node_type;
    typedef typename root_t::index_type index_type;
...
protected:
    ...
    struct deref_node_t {
        node_type &operator()(size_type index) const { return config_t::get_node(*container_ptr, index); }

        container_type *container_ptr;
    };

    struct const_deref_node_t {
        node_type const &operator()(size_type index) const { return config_t::get_node(*container_ptr, index); }
    ...
    struct get_key_select_t {
        ...
        key_type const &operator()(storage_type const &value) const { return config_t::get_key(value); }
    ...
    struct get_key_select_t<k_t, k_t> {
        key_type const &operator()(key_type const &value) const { return config_t::get_key(value); }
...
public:
    ...
    public:
        explicit const_reverse_iterator(threaded_rbtree_impl const *_tree, size_type _where)
...
public:
    typedef typename std::conditional<config_t::unique_type::value, std::pair<iterator, bool>, iterator>::type
        insert_result_t;
...
public:
    ...
    threaded_rbtree_impl(iterator_t begin, iterator_t end, key_compare const &comp = key_compare(),
    ...
    // move
    threaded_rbtree_impl(threaded_rbtree_impl &&other) : root_(key_compare(), container_type()) { swap(other); }

    // move
    threaded_rbtree_impl(threaded_rbtree_impl &&other, container_type const &container)
        : root_(key_compare(), container) {
        insert(std::make_move_iterator(other.begin()), std::make_move_iterator(other.end()));
    }

    // initializer list
    threaded_rbtree_impl(std::initializer_list<value_type> il, key_compare const &comp = key_compare(),
                         container_type const &container = container_type())
    ...
    typename std::enable_if<std::is_convertible<in_value_t, value_type>::value, insert_result_t>::type insert(
        in_value_t &&value) {
        check_max_size_();
        return result_<typename config_t::unique_type>(
            trb_insert_(typename config_t::unique_type(), std::forward<in_value_t>(value)));
    }
    ...
    template <class in_value_t>
    typename std::enable_if<std::is_convertible<in_value_t, value_type>::value, insert_result_t>::type insert(
        const_iterator hint, in_value_t &&value) {
        check_max_size_();
        return result_<typename config_t::unique_type>(
            trb_insert_(typename config_t::unique_type(), std::forward<in_value_t>(value)));
    }
    ...
    insert_result_t emplace(args_t &&... args) {
        check_max_size_();
        return result_<typename config_t::unique_type>(
            trb_insert_(typename config_t::unique_type(), std::forward<args_t>(args)...));
    }
    ...
    iterator find(key_type const &key) {
        if (config_t::unique_type::value) {
            return iterator(this, threaded_rbtree_equal_unique(root_, const_deref_node_t{&root_.container}, key,
                                                               deref_key_t{&root_.container}, get_comparator_()));
            ;
        } else {
            size_type where = threaded_rbtree_lower_bound(root_, const_deref_node_t{&root_.container}, key,
                                                          deref_key_t{&root_.container}, get_comparator_());
            return iterator(this, (where == node_type::nil_sentinel || get_comparator_()(key, get_key_(where)))
    ...
    const_iterator find(key_type const &key) const {
        if (config_t::unique_type::value) {
            return const_iterator(this, threaded_rbtree_equal_unique(root_, const_deref_node_t{&root_.container}, key,
                                                                     deref_key_t{&root_.container}, get_comparator_()));
        } else {
            size_type where = threaded_rbtree_lower_bound(root_, const_deref_node_t{&root_.container}, key,
                                                          deref_key_t{&root_.container}, get_comparator_());
            return const_iterator(this, (where == node_type::nil_sentinel || get_comparator_()(key, get_key_(where)))
                                            ? node_type::nil_sentinel
                                            : where);
        };
    }
    ...
    iterator lower_bound(key_type const &key) { return iterator(this, lwb_i(key)); }

    const_iterator lower_bound(key_type const &key) const { return const_iterator(this, lwb_i(key)); }

    iterator upper_bound(key_type const &key) { return iterator(this, upb_i(key)); }

    const_iterator upper_bound(key_type const &key) const { return const_iterator(this, upb_i(key)); }

    pair_ii_t equal_range(key_type const &key) {
        size_type lower, upper;
        threaded_rbtree_equal_range(root_, const_deref_node_t{&root_.container}, key, deref_key_t{&root_.container},
                                    get_comparator_(), lower, upper);
    ...
    size_type beg_i() const { return root_.get_most_left(const_deref_node_t{&root_.container}); }

    size_type end_i() const { return node_type::nil_sentinel; }

    size_type rbeg_i() const { return root_.get_most_right(const_deref_node_t{&root_.container}); }

    size_type rend_i() const { return node_type::nil_sentinel; }

    size_type next_i(size_type i) const { return threaded_rbtree_move_next(i, const_deref_node_t{&root_.container}); }

    size_type prev_i(size_type i) const { return threaded_rbtree_move_prev(i, const_deref_node_t{&root_.container}); }

    size_type lwb_i(key_type const &key) const {
        return threaded_rbtree_lower_bound(root_, const_deref_node_t{&root_.container}, key,
                                           deref_key_t{&root_.container}, get_comparator_());
    }

    size_type upb_i(key_type const &key) const {
        return threaded_rbtree_upper_bound(root_, const_deref_node_t{&root_.container}, key,
                                           deref_key_t{&root_.container}, get_comparator_());
    }

    size_type rlwb_i(key_type const &key) const {
        return threaded_rbtree_reverse_lower_bound(root_, const_deref_node_t{&root_.container}, key,
                                                   deref_key_t{&root_.container}, get_comparator_());
    }

    size_type rupb_i(key_type const &key) const {
        return threaded_rbtree_reverse_upper_bound(root_, const_deref_node_t{&root_.container}, key,
                                                   deref_key_t{&root_.container}, get_comparator_());
    }

    key_type const &key(size_type i) const { return config_t::get_key(config_t::get_value(root_.container, i)); }

    value_type &elem_at(size_type i) { return reinterpret_cast<value_type &>(config_t::get_value(root_.container, i)); }
    ...
    const_reverse_iterator rbegin() const { return const_reverse_iterator(this, rbeg_i()); }

    const_reverse_iterator rend() const { return const_reverse_iterator(this, rend_i()); }

    const_reverse_iterator crbegin() const { return const_reverse_iterator(this, rbeg_i()); }

    const_reverse_iterator crend() const { return const_reverse_iterator(this, rend_i()); }
    ...
    container_type &container() { return root_.container; }

    container_type const &container() const { return root_.container; }

protected:
    root_t root_;

protected:
    key_compare &get_comparator_() { return root_; }
    ...
    pair_posi_t trb_insert_(std::false_type, arg_first_t &&arg_first, args_other_t &&... args_other) {
        ...
        threaded_rbtree_find_path_for_multi(root_, stack, deref_node_t{&root_.container}, index,
    ...
    pair_posi_t trb_insert_(std::true_type, arg_first_t &&arg_first, args_other_t &&... args_other) {
        ...
        bool exists = threaded_rbtree_find_path_for_unique(root_, stack, deref_node_t{&root_.container}, key,
                                                           deref_key_t{&root_.container}, get_comparator_());
        if (exists) {
    ...
    void trb_erase_index_(size_type index) {
        threaded_rbtree_stack_t<node_type, stack_max_depth> stack;
        bool exists = threaded_rbtree_find_path_for_remove(root_, stack, deref_node_t{&root_.container}, index,
                                                           key_compare_ex{&root_});
        assert(exists);
...
struct threaded_rbtree_default_set_config_t {
    ...
    static node_type &get_node(container_type &container, std::size_t index) { return container[index].node; }
...
using trb_set =
    threaded_rbtree_impl<threaded_rbtree_default_set_config_t<key_t, comparator_t, index_t, std::true_type>>;

template <class key_t, class comparator_t = std::less<key_t>, class index_t = uint32_t>
using trb_multiset =
    threaded_rbtree_impl<threaded_rbtree_default_set_config_t<key_t, comparator_t, index_t, std::false_type>>;

template <class key_t, class value_t, class comparator_t = std::less<key_t>, class index_t = uint32_t>
class trb_map : public threaded_rbtree_impl<
                    threaded_rbtree_default_map_config_t<key_t, value_t, comparator_t, index_t, std::true_type>> {
...
public:
    ...
    explicit trb_map(typename base_t::container_type const &container) : base_t(container) {}
    template <class... args_t>
    ...
    typename base_t::mapped_type &operator[](typename base_t::key_type const &key) {
        ...
        if (offset == base_t::node_type::nil_sentinel ||
            base_t::get_comparator_()(key, typename base_t::deref_key_t{&base_t::root_.container}(offset))) {
...
template <class key_t, class value_t, class comparator_t = std::less<key_t>, class index_t = uint32_t>
using trb_multimap =
    threaded_rbtree_impl<threaded_rbtree_default_map_config_t<key_t, value_t, comparator_t, index_t, std::false_type>>;
