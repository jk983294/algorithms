...
public:
    ...
    inline void swap(int i,int j){T tmp = data[i];    data[i] = data[j];  data[j] = tmp;};
...
bool SortAlgorithm<T>::binarySearch(T key){
    ...
    while(left <= right){
        ...
        else if(data[middle] < key) left = middle + 1;
        else if(data[middle] > key) right = middle - 1;
        middle = (left + right) / 2;
    }
    return false;
}
...
void SortAlgorithm_test(){
    cout<<"test SortAlgorithm start..."<<endl;
    testInsertSort();
    testShellSort();
    testQuickSort();
    testBubbleSort();
    ...
    sa.heapSort();
    sa.print();
    cout<<sa.binarySearch(26)<<endl;
    cout<<sa.binarySearch(25)<<endl;

    cout<<sa.binarySearchForIndex(26)<<endl;
    cout<<sa.binarySearchForIndex(64)<<endl;
    cout<<"test SortAlgorithm end..."<<endl;
}
