...
public:
    ...
    void push(int x) {
        if (this->isFull()) {
            ...
            /**
             * 如果压入堆栈的值比之前记录的最大值大，那么那之前的最大值记录在link2NextMaxItem[stackTop]的位置
             * 把当前最大值在堆栈stackItem中的位置用maxStackItemIndex保存
             */
            if (x > maxValue()) {
    ...
    int pop() {
        ...
        if (this->isEmpty()) {
            ...
            /**
             * 如果pop出的值是当前的最大值，那么则需要将之前保存到link2NextMaxItem[stackTop]中的最大值取出
             * 更新maxStackItemIndex的值
             */
...
class QueueWithMax {
public:
    int maxValue(int x, int y) { return x > y ? x : y; }

    // 返回队列中最大值
    int max() { return maxValue(stackA.maxValue(), stackB.maxValue()); }
    ...
    /**
     * 删除并返回队首元素,如果stackA中是空，则先将stackB中的所有元素pop到stackA中，
